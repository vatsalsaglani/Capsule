...
- [`docs/learnings/2026-07-14-branch-release-pages.md`](docs/learnings/2026-07-14-branch-release-pages.md) — branch-only release UX still requires a workflow-managed Git tag; gate the exact `main` CI SHA, keep full prerelease SemVer outside `CFBundleShortVersionString`, tag a deterministic version-only child commit, and set explicit pip cache paths with a docs-only recovery route
