...
- [`docs/learnings/2026-07-14-default-kernel-readiness.md`](docs/learnings/2026-07-14-default-kernel-readiness.md) — a running apiserver does not imply container-create readiness; mirror Apple 1.1's resolved `appRoot/kernels/default.kernel-<arch>` check, never treat archive-internal `kernel.binaryPath` as a host path, and keep kernel setup manual
- [`docs/learnings/2026-07-14-runtime-xpc-outage-presentation.md`](docs/learnings/2026-07-14-runtime-xpc-outage-presentation.md) — a stopped/unresponsive Apple container 1.1 service can surface as `XPC connection error: Connection invalid`; distinguish it from binary absence, offer native start recovery, and keep transport text behind technical details
...
- [`docs/learnings/2026-07-14-branch-release-pages.md`](docs/learnings/2026-07-14-branch-release-pages.md) — branch-only release UX still requires a workflow-managed Git tag; gate the exact `main` CI SHA, keep full prerelease SemVer outside `CFBundleShortVersionString`, deploy post-release docs corrections from exact reviewed SHAs without moving immutable artifacts, account for Pages' ten-minute HTML cache, and keep Pages open to `main` plus `release/*`
