...
| S4 stats | `container stats` format and per-second cost of polling 10 containers? | **decided 2026-07-13 — trimmed: format/stream question already resolved by P1A (one-shot streaming, poll loop over `--no-stream` implemented); this spike measured cost — `--no-stream` has a fixed ~2.2s wall-clock latency floor independent of N (1/5/10 all ~2.2-2.4s), dominated by round-trip wait not CPU (apiserver 0.0% CPU under sustained polling). Recommend keeping the 2s `statsInterval` default (real cadence ≈4.2s, call+sleep), pausing the poll when the sparkline UI isn't visible, no dedicated stats actor (reuse `stats(ids:)` directly), no contention with the `ls` Poller. See [spike](S4-stats-cost.md) and [learnings](../learnings/2026-07-13-stats-polling-cost.md).** |
| S5 build cancel | SIGINT/SIGTERM semantics of `container build`; does cancellation leave builder state consistent? | **decided 2026-07-13 — SIGTERM alone suffices (no orphan/wedge observed across SIGINT/SIGTERM/even SIGKILL); recommend a general 5s-grace SIGTERM→SIGKILL `Subprocess` contract as defense-in-depth (not build-specific); `--progress plain` confirmed line-parseable. See [spike](S5-build-cancel.md) and [learnings](../learnings/2026-07-13-build-cancellation.md).** |
