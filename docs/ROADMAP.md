...
- [x] `capsule doctor` (binary, version, apiserver status, default-kernel readiness, GitHub update check)
...
- [x] Runtime install/update UX: onboarding screen when CLI missing → download latest release `.pkg` from GitHub, guide install; update banner when `RuntimeUpdateChecker` finds a newer release *(P1D — `RuntimeUpdateChecker.evaluate()` pure comparison + `Sources/RuntimeInstaller/RuntimeInstallerModel` (presence/update-status/download-and-handoff, never executes the `.pkg`, injectable runtime/fetch/download seams) + `capsule runtime status` CLI + `App/Capsule/Onboarding/{OnboardingView,UpdateBanner,InstallerHandoffView}` wired at the app root into `RootView`/`CapsuleApp`; downloaded packages now explicitly require Reveal → double-click `.pkg` → complete Installer → Check Again, and binary absence owns the full window. All engine/store/compile-level complete against the real 1.1.0 runtime and the real GitHub API shape (verified live, see learnings) — **still open human gates:** an actual onboarding-screen click-through, a real `.pkg` download + running the installer, and the update banner appearing against a genuinely newer release)*
...
- [ ] Design pass to plan §6 spec (motion, typography, color tokens, translucency rules) *(Compose, Containers, Images, Volumes, and Networks now share Graphite & Indigo tokens, adaptive card/list modes, hover-local actions, selected states, exact destructive previews, and solid log surfaces. Runtime-missing onboarding is full-window; service outages use red/green Capsule status marks in the title bar/sidebar, a native Start Runtime recovery, and collapsed technical details; empty Compose projects use one full-content import target instead of an empty split. The human frame-by-frame gate remains.)*
